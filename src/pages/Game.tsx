...
export default function Game() {
    ...
    const players : 1 | 2 | 3 | 4 = location.state.players
    ...
    }, [characters, correctPairs])
    
    // Current Player logic (if there are more than one player)
    const [currentPlayer, setCurrentPlayer] = React.useState< 1 | 2 | 3 | 4 >(1)
    const [player1Score, setP1Score] = React.useState<number>(0)
    const [player2Score, setP2Score] = React.useState<number>(0)
    const [player3Score, setP3Score] = React.useState<number>(0)
    const [player4Score, setP4Score] = React.useState<number>(0)

    let playerElements = []

    for (let i = 1; i <= players; i++) {
        const playerNumber = i;
        const scoreState : number = eval(`player${playerNumber}Score`);
        playerElements.push(<Player player={i} score={scoreState} currentPlayer={currentPlayer} />);
    }
    console.log(playerElements)

    return (

        <main className="bg-white3 p-4 h-screen flex flex-col justify-between">
            <header className="flex justify-between items-center">
                <div className="svg-container-game mt-2" aria-label="This is the logo for the game.">
