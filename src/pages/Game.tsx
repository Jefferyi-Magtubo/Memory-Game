        ...
        <main className="bg-white3 p-4 h-screen">
            ...
            <section className={`grid ${gridSize === "4x4" ? "grid-cols-4" : gridSize === "6x6" ? "grid-cols-6" : ""} gap-4 mt-12`}>
                {characters?.map((char, index) => {
                    const key = nanoid()
                    return <Circle key={key} customKey={key} character={char.character} clicked={char.clicked} theme={theme} changeClick={changeClick} index={index} correctPairs={correctPairs}/>
                } )}
            </section>
